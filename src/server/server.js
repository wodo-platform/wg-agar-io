...
// var WodoApiClient = require("./lib/wodoApiClient");
var GameLogUtil = require("./lib/gameLogUtil");
...
// var wodoApiClient = new WodoApiClient();
var gameLogUtil = new GameLogUtil();
...
io.on('connection', function(socket) {
    ...
    socket.on('gotit', function(player) {
        console.log('[INFO] Player ' + player.name + ' with userId: ' + player.userId + ' connecting!');
        ...
        if (util.findIndex(users, player.id) > -1) {
            ...
            console.log('[INFO] Player ' + player.name + ' with userId: ' + player.userId + ' and bid  ' + player.playerBid + ' connected!');
            sockets[player.id] = socket;

            // wodoApiClient.joinGame(player.name, player.userId, player.playerBid, gameLogUtil);
    ...
    socket.on('end', function(data) {
        console.log("ending the game:" + JSON.stringify(data));
        console.log("users -->" + JSON.stringify(users));
        console.log("leaderBoard -->" + JSON.stringify(leaderboard));
        leaderboard.forEach(user => {
            gameLogUtil.addUser2LeaderBoard(user.userId, user.name, user.score);
            if (sockets[user.id]) {
                sockets[user.id].emit('RIP');
            }
        });
        gameLogUtil.addGameServerShutDownEvent();

        console.log("game log -->" + JSON.stringify(gameLogUtil.gameServerLog));
        // wodoApiClient.updateGameServer(gameLogUtil);
...
function tickPlayer(currentPlayer) {
        ...
        if (collision.aUser.mass > collision.bUser.mass * 1.1 && collision.aUser.radius > Math.sqrt(Math.pow(collision.aUser.x - collision.bUser.x, 2) + Math.pow(collision.aUser.y - collision.bUser.y, 2)) * 1.75) {
            ...
            if (numUser > -1) {
                if (users[numUser].cells.length > 1) {
                    ...
                    gameLogUtil.addEventGameScore("userScored", currentPlayer.userId, collision.bUser.userId, collision.bUser.mass);
            ...
            console.log("server log-->" + JSON.stringify(gameLogUtil.gameServerLog));
    ...
    for (var z = 0; z < currentPlayer.cells.length; z++) {
        ...
        var otherUsers = tree.get({ x: currentCell.x, y: currentCell.y, w: currentCell.radius * 8, h: currentCell.radius * 8 }, check);
...
function createGameServer() {
    gameLogUtil.instantiate(null, 1, 1);
    gameLogUtil.addGameServerBootstrapEvent();

    // wodoApiClient.createGameServer(gameLogUtil);
    /* axios.get('http://localhost:3001/api/game-servers').then((response) => {
