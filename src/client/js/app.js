...
var playerBidInput = document.getElementById('playerBidInput');
...
function startGame(type) {
    ...
    global.playerBid = playerBidInput.value;
    var userId = 1;
    if (global.playerName == "candan") {
        userId = 1;
...
function setupSocket(socket) {
    ...
    socket.on('welcome', function(playerSettings) {
        ...
        player.playerBid = global.playerBid;
        player.userId = global.userId;
        ...
        for (var i = 0; i < leaderboard.length; i++) {
            ...
            if (leaderboard[i].id == player.id) {
                if (leaderboard[i].name.length !== 0)
                    status += '<span class="me">' + (i + 1) + '. ' + leaderboard[i].name + ' - score: ' + leaderboard[i].score + "</span>";
                else
                    status += '<span class="me">' + (i + 1) + ". An unnamed cell</span>";
            } else {
                if (leaderboard[i].name.length !== 0)
                    status += (i + 1) + '. ' + leaderboard[i].name + ' - score:' + leaderboard[i].score;
                else
